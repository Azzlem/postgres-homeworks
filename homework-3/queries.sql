...
select customers.company_name, (employees.first_name || ' ' || employees.last_name) as FIO
from orders
...
-- 2. Наименование продукта, количество товара (product_name и units_in_stock в табл products),q
...
from products
where exists (select quantity from order_details where product_id=products.product_id and quantity=10);
